...
INTRO_TEXT = 'Dzień dobry, mówi Maja, asystentka AI Konrada Kucharskiego. Dzwonię w jego imieniu.'
...
async def health():
    ...
    return JSONResponse({'ready':ready, 'intro_ready':bool(INTRO_AUDIO), 'version':'2026-09-09-natural-intro-cost-context','languages_ready':(['pl'] if INTRO_AUDIO else [])+[c for c in INTRO_AUDIOS if c!='pl'], 'features':['scheduled_calls','transcript','owner_chat']},status_code=200 if ready else 503)
...
O kosztach rozmawiaj wyłącznie, gdy cena, opłata lub oferta kosztowa rzeczywiście występuje w zakresie sprawy albo w wypowiedzi rozmówcy. Przy zwykłym spotkaniu nie pytaj o cenę i nie mów o zerowym koszcie, 0,00 PLN ani bezpłatności. Techniczne amount_grosz=0 w check_offer nie jest ustaloną ceną i nie wolno go odczytywać ani dopisywać do opisu, podsumowania lub pytania. Gdy nie ma tematu finansowego, wpisz technicznie 0 i opisuj tylko rzeczywiste ustalenia. Jeśli pojawi się rzeczywista cena, ustal ją i sprawdź zgodę właściciela. Przy rzeczywistej ofercie cenowej podaj pełną kwotę brutto w groszach, PLN i dokładny termin. Nie dziel kwot ani nie pomijaj opłat.
